...
export function OutstandingBalanceByStudioChart({ byStudio = [] }) {
  if (!byStudio.length) return null

  return (
    <section className={chartCardClass}>
      <h3 className="text-base font-bold uppercase tracking-[0.02em] text-[var(--studio-primary)]">
        Outstanding Balance by Studio
      </h3>
      <div className="mt-4 h-[220px]">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={byStudio} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
            <CartesianGrid stroke={chartGridStroke} vertical={false} />
            <XAxis dataKey="studio" tick={{ fill: chartAxisStroke, fontSize: 12 }} tickLine={false} axisLine={false} />
            ...
            <Bar dataKey="amount" name="Outstanding" radius={[8, 8, 0, 0]} fill="url(#balanceBarGradient)" />
            <defs>
