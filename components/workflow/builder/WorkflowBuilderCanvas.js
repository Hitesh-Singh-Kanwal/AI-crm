...
function CanvasInner() {
  ...
  return (
    <div ref={reactFlowWrapper} className="relative h-full w-full">
      <div className="pointer-events-none absolute left-4 top-4 z-10 flex max-w-[min(100%,22rem)] items-center gap-2 rounded-full border border-border/80 bg-white/95 px-3 py-1.5 text-[11px] text-muted-foreground shadow-sm backdrop-blur">
        <MousePointer2 className="h-3.5 w-3.5 shrink-0" />
        Click a wire to disconnect · Steps stay on the board to reconnect anywhere
      </div>

      {selectedEdgeId ? (
        <div className="absolute left-1/2 top-4 z-10 flex -translate-x-1/2 items-center gap-2 rounded-xl border border-border/80 bg-white/95 p-1 shadow-sm backdrop-blur">
          <button
            type="button"
      ...
      <div className="absolute right-4 top-4 z-10 flex items-center gap-1.5 rounded-xl border border-border/80 bg-white/95 p-1 shadow-sm backdrop-blur">
        <button
          type="button"
          onClick={() => setSidebarCollapsed(!sidebarCollapsed)}
          title={sidebarCollapsed ? 'Show steps panel' : 'Hide steps panel'}
          className={cn(
            'inline-flex h-8 w-8 items-center justify-center rounded-lg text-muted-foreground transition-colors hover:bg-muted hover:text-foreground dark:hover:bg-muted',
        ...
        <button
          ...
          className={cn(
            'inline-flex h-8 items-center gap-1.5 rounded-lg px-2.5 text-[11px] font-semibold text-muted-foreground transition-colors hover:bg-muted hover:text-foreground dark:hover:bg-muted',
      ...
      <div className="pointer-events-none absolute bottom-[11.25rem] left-4 z-10 flex items-center gap-1.5 rounded-t-xl border border-b-0 border-border/90 bg-card px-3 py-1.5 shadow-sm">
      ...
      <button
        ...
        className="absolute bottom-4 right-4 z-10 inline-flex items-center gap-1.5 rounded-xl border border-border/90 bg-card px-3.5 py-2 text-[12px] font-semibold text-foreground shadow-md transition-colors hover:border-border hover:bg-muted/40 dark:hover:bg-muted"
      >
      ...
      >
        ...
        <Controls
          ...
          className="!bottom-[4.25rem] !right-4 !m-0 !rounded-xl !border !border-border/90 !bg-card !shadow-md [&>button]:!h-8 [&>button]:!w-8 [&>button]:!border-border [&>button]:!bg-card [&>button]:!text-foreground [&>button]:hover:!bg-muted/40"
        />
        <MiniMap
          nodeStrokeWidth={2}
          nodeBorderRadius={6}
          ...
          className="workflow-minimap !bottom-4 !left-4 !m-0 !rounded-b-xl !rounded-t-none overflow-hidden !border !border-border/90 !border-t-0 !bg-card !shadow-lg dark:!border-border dark:!bg-card"
