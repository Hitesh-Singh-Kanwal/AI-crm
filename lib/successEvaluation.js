...
function parseAutomaticRubric(text) {
  ...
  if (overallMeta?.value) {
    const overallMatch = overallMeta.value.match(/(\d+(?:\.\d+)?)\s*(?:\/\s*(\d+))?/)
    if (overallMatch) {
      overallScore = Number(overallMatch[1])
      overallMax = Number(overallMatch[2] || 10)
    }
  }
...
export function parseSuccessEvaluation(raw, rubricHint) {
  ...
  if (text.length < 24 && (PASS_VALUES.has(lower) || FAIL_VALUES.has(lower))) {
    return {
      type: 'passFail',
      raw: text,
      passed: PASS_VALUES.has(lower),
      label: PASS_VALUES.has(lower) ? 'Pass' : 'Fail',
    }
  }
  ...
  if (rubric === 'AutomaticRubric' || (rubric !== 'Matrix' && /\/\s*10\s*[—–-]/i.test(text))) {
    const automatic = parseAutomaticRubric(text)
  ...
  if (rubric === 'PercentageScale') {
    const pctInline = text.match(/(\d+(?:\.\d+)?)\s*%/)
    if (pctInline) {
  ...
  if (rubric === 'NumericScale') {
    const numericOnly = text.match(/^(\d+(?:\.\d+)?)(?:\s*\/\s*(\d+))?$/)
    if (numericOnly) {
      return {
        type: 'numeric',
        raw: text,
  ...
  if (rubric === 'DescriptiveScale' || rubric === 'LikertScale') {
    const scale = parseScaleRubric(text, rubric)
  ...
  const automaticFallback = parseAutomaticRubric(text)
