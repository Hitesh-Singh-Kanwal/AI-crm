 ...
 * calendar-based items (lessons/services/memberships/to-dos), lead and
 * customer callback dates, and calls waiting on human intervention.
 *
 * - Calendar items: non-admin users only see events where they're the
 *   assigned teacher; admins/super-admins see everything org-wide. To-dos
 *   are shown to everyone regardless of assignment.
  ...
  const fetch = useCallback(async ({ silent = false } = {}) => {
    ...
    try {
      ...
      const calendarItems = !calendarResult.success || !Array.isArray(calendarResult.data)
        ...
        .filter((evt) => {
          // To-dos aren't personal bookings — everyone should see them,
          // regardless of who they're assigned to on the calendar.
          if (deriveKind(evt) === 'todo') return true
          if (isAdmin || !userId) return true
          const teacherId = String(evt.teacherID?._id || evt.teacherID || '')
          const teacherIds = Array.isArray(evt.teacherIDs)
            ? evt.teacherIDs.map((t) => String(t?._id ?? t))
            : []
          return teacherId === userId || teacherIds.includes(userId)
        })
        .map((evt) => {
