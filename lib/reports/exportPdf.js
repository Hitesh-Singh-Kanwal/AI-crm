...
export function buildReportPdfHtml(rows, columns, title) {
  const header = columns.map((c) => `<th>${pdfEscape(c.label)}</th>`).join('')
  const body = rows
    .map(
      (row) =>
        `<tr>${columns
          .map((c) => `<td>${pdfEscape(formatReportCellValue(row[c.key], c))}</td>`)
          .join('')}</tr>`
    )
    .join('')
...
export function exportCurrentPageToPdf(rows, columns, title, filename = 'report.pdf') {
  const html = buildReportPdfHtml(rows, columns, title)
