...
/** Known lead reason codes that need a specific label rather than generic Title Case. */
const REASON_LABEL_OVERRIDES = {
  wedding_dance: 'Wedding Dance',
  special_event: 'Special Event',
  stay_in_shape: 'Stay in Shape',
  stay_in_shap: 'Stay in Shape',
  meet_new_people: 'Meet New People',
  try_something_new: 'Try Something New',
  have_fun: 'Have Fun',
}

/** Built-in reasons — mirror backend SYSTEM_LEAD_REASON_DEFS. */
export const SYSTEM_REASON_CODES = new Set([
  'special_event',
  'stay_in_shape',
  'stay_in_shap',
  'meet_new_people',
...
export function isSystemLeadReason(reason) {
  if (!reason) return false
  if (reason.isSystem) return true
  const code = String(reason.reasonCode || reason.value || reason.name || '')
    .trim()
    .toLowerCase()
    .replace(/\s+/g, '_')
  return SYSTEM_REASON_CODES.has(code)
}

/**
 * Lead/customer reason for UI: `special_event` → "Special Event".
 * Prefer explicit overrides, then catalog labels, then Title Case of the code.
 */
export function formatReasonLabel(reason, leadReasons = []) {
  ...
  if (REASON_LABEL_OVERRIDES[codeKey]) return REASON_LABEL_OVERRIDES[codeKey]

  const match = getLeadReasonOptions(leadReasons).find(
    (o) => String(o.value).trim() === code || String(o.value).trim().toLowerCase() === codeKey,
  )
  const catalogLabel = String(match?.label || '').trim()
  if (catalogLabel) {
    const catalogKey = catalogLabel.toLowerCase()
    if (REASON_LABEL_OVERRIDES[catalogKey]) return REASON_LABEL_OVERRIDES[catalogKey]
    // Catalog stored the code as the name (e.g. "special_event") — humanize it.
    if (/[_-]/.test(catalogLabel) || catalogLabel.toLowerCase() === codeKey) {
      return formatFieldDisplayValue(catalogLabel)
    }
    return catalogLabel
  }

  return formatFieldDisplayValue(code)
}
...
export function findCatalogGroupIdForField(field, entityType = 'lead') {
  if (!field) return ''
  const catalogGroups = entityType === 'customer' ? CUSTOMER_FILTER_GROUPS : FILTER_GROUPS
  return catalogGroups.find((g) => g.fields.some((f) => f.value === field))?.id || ''
}

export function inferCatalogGroupId(group, entityType = 'lead') {
  if (group?.catalogGroupId) return group.catalogGroupId
  for (const condition of group?.conditions || []) {
    const resolved = findCatalogGroupIdForField(condition?.field, entityType)
...
export function buildConditionGroupsFromFlat({
  ...
  if (Array.isArray(conditionGroups) && conditionGroups.length > 0) {
    return conditionGroups.map((g, index) => {
      const normalizedConditions = normalizeConditionsForForm(g.conditions || []).map((c) => ({
        ...c,
        groupId: g.id || c.groupId,
      }))
      const catalogGroupId = inferCatalogGroupId(
        { catalogGroupId: g.catalogGroupId, conditions: normalizedConditions },
        entityType
      )
      return {
  ...
  return Array.from(byGroup.entries()).map(([groupId, groupConditions], index) => {
    const catalogGroup =
      findCatalogGroupIdForField(groupConditions[0]?.field, entityType) ||
...
export function buildDynamicListPayload(form) {
  ...
  if (byGroup.size > 1 || Object.keys(groupLogics).length > 0) {
    if (Array.isArray(form.conditionGroups) && form.conditionGroups.length > 0) {
      payload.conditionGroups = form.conditionGroups.map((group) => ({
        id: group.id,
        ...(group.catalogGroupId ? { catalogGroupId: group.catalogGroupId } : {}),
...
export function summarizeCondition(condition, context = {}, entityType = 'lead') {
  ...
  const formatValue = (v) =>
    entityType === 'customer'
      ? formatCustomerConditionValueLabel(condition?.field, v, {
          locations,
          teachers,
          tags,
          memberships,
