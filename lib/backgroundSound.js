...
export function soundMatchesValue(sound, value) {
  ...
  if (orgId && sound.fileID) {
    ...
    const suffixes = [
      `/public/background-sounds/${orgId}/${sound.fileID}`,
      `/background-sounds/${orgId}/${sound.fileID}`,
    ]
    if (suffixes.some((suffix) => normalized.includes(suffix) || v.includes(suffix))) return true
  }
  return false
}
