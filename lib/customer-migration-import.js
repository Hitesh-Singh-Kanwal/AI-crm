...
function tagColumnsForUniversalSheet(columns, group, renameMap = {}) {
  return columns.map((c) => {
    ...
    // the mapping-review UI and sample file, not just internally-distinct keys.
    const headers = renameMap[c.key] ? [`${group} ${c.headers[0]}`, ...c.headers] : c.headers
    return { ...c, key: newKey, group, originalKey: c.key, headers }
  })
}

export const UNIVERSAL_COLUMNS = [
  ...tagColumnsForUniversalSheet(CUSTOMER_COLUMNS, 'customer'),
  ...tagColumnsForUniversalSheet(ENROLLMENT_COLUMNS, 'enrollment', UNIVERSAL_RENAMES.enrollment).filter(
...
export function splitUniversalRows(rows) {
  ...
  for (const row of rows) {
    ...
    if (custKey && !seenCustomerKeys.has(custKey)) {
    ...
    if (!isBlankCell(row.programName) && !isBlankCell(row.sessionsTotal)) enrollments.push(pick(row, 'enrollment'))
    if (!isBlankCell(row.membershipPlanName)) memberships.push(pick(row, 'membership'))
...
export function downloadSampleXlsx(filename = 'sample_migration_import.xlsx') {
  const headers = UNIVERSAL_COLUMNS.map((c) => c.headers[0])
  const row1 = UNIVERSAL_COLUMNS.map((c) => c.sample ?? '')
  const row2 = UNIVERSAL_COLUMNS.map((c) => {
...
export function downloadUniversalSampleCsv(filename = 'sample_migration_import.csv') {
  downloadSampleCsv(UNIVERSAL_COLUMNS, filename)
}
