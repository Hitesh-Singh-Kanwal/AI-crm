...
/** Payload for POST /api/email/send-one.
 * Typed compose → `text` only (no HTML wrap). Template compose → `html`/`body`.
 */
export function buildSendOneEmailPayload({
  ...
  const textTrimmed = String(content || '').trim()
  ...
  if (htmlTrimmed) {
    // Designed / template HTML — keep as html+body for layout + analytics.
    const footerSaved =
      hasFooter === true || hasFooter === false
        ? Boolean(hasFooter)
        : htmlHasStudioFooter(htmlTrimmed)
    payload.html = htmlTrimmed
    payload.body = htmlTrimmed
    if (useTemplate) {
    ...
    // Plain typed message — send as text so it is not wrapped in <p>/<br> tags.
    // Backend converts to HTML for delivery (footer) while keeping a text part.
    payload.text = textTrimmed
  }
